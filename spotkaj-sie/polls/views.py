...
def show_plans(request):
    ...
    """
    if request.user.is_authenticated:
        terms = Plan.objects.all()

        return render(request, 'polls/show_plans.html', {'terms': terms})
    return render(request,
                  'polls/user_profile.html',
                  {'error_message': "Musisz być zalogowany!"})
...
def show_events(request):
    ...
    if request.user.is_authenticated:
        ...
        for i in events:
            i.duration = int(i.duration // 60)
            print( i.found_time)

        return render(request, 'polls/show_events.html', {'events': events})
    return render(request,
                  'polls/user_profile.html',
                  {'error_message': "Musisz być zalogowany!"})
